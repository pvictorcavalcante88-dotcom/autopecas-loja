...
async function limparBancoDeTestes() {
    ...
    // 🔴 A CORREÇÃO ESTÁ AQUI: Mudamos de 'token' para 'adminToken'
    const token = localStorage.getItem('adminToken'); 

    if (!token) {
        alert("Erro: Você não parece estar logado como Admin.");
        return;
    }
    ...
    try {
        const res = await fetch('/admin/limpar-banco-testes', {
            method: 'DELETE',
            headers: { 
                'Authorization': `Bearer ${token}`, // Agora vai o token certo
                'Content-Type': 'application/json'
            }
        });

        // Verificação de segurança antes de tentar ler JSON
        if (res.status === 403 || res.status === 401) {
            alert("❌ Acesso negado! Verifique se você está logado como Admin.");
            return;
        }
        ...
        if(res.ok) {
            ...
            alert("❌ Erro: " + (dados.erro || "Falha ao limpar"));
        }
    } catch (e) {
        console.error(e);
