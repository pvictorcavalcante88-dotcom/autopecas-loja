...
// ==============================================================
// 📦 LÓGICA DA PÁGINA DE PRODUTO (product.html)
// ==============================================================

// Variáveis Globais do Produto
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // 1. Busca Global (Funciona em todas as páginas)
    const btnSearch = document.getElementById('search-button');
    const inputSearch = document.getElementById('search-input');

    if(btnSearch) btnSearch.addEventListener('click', realizarBusca);
    if(inputSearch) inputSearch.addEventListener('keypress', (e) => { 
        if(e.key === 'Enter') realizarBusca(); 
    });

    // 2. Verifica se estamos na página de produto (tem 'product-container'?)
    const containerProduto = document.getElementById('product-container');
    
    if (containerProduto) {
        // Estamos na página do produto!
        const params = new URLSearchParams(window.location.search);
        const id = params.get('id');
        
        if(id) {
            carregarProdutoDetalhado(id);
        } else {
            containerProduto.innerHTML = '<h2 style="text-align:center; padding:50px;">Produto não especificado.<br><a href="index.html" class="btn-buy-now" style="display:inline-block; margin-top:20px; width:auto;">Voltar para Loja</a></h2>';
        }
        
        // Listeners específicos da página de produto
        const qtdInput = document.getElementById('qtd');
        if(qtdInput) qtdInput.addEventListener('input', atualizarPrecoTela);
        
        const margemInput = document.getElementById('input-margem');
        if(margemInput) {
            margemInput.addEventListener('input', (e) => {
                let valor = parseFloat(e.target.value);
                if(valor < 0) { valor = 0; e.target.value = 0; }
                MARGEM_ATUAL = valor || 0;
                atualizarPrecoTela();
            });
        }
    }
...
// --- FUNÇÕES GLOBAIS (Podem ser chamadas pelo HTML onclick) ---
...
async function carregarProdutoDetalhado(id) {
    try {
        // 1. DADOS DO AFILIADO E MARGEM
        const afData = localStorage.getItem('afiliadoLogado');
        if (afData) {
            MARGEM_ATUAL = parseFloat(localStorage.getItem('minhaMargem') || 0);
            const areaAfiliado = document.getElementById('affiliate-area');
            const inputMargem = document.getElementById('input-margem');
            const boxSugestao = document.getElementById('box-sugestao');
            
            if(areaAfiliado) areaAfiliado.style.display = 'block';
            if(inputMargem) inputMargem.value = MARGEM_ATUAL;
            if(boxSugestao) boxSugestao.style.display = 'block';
        }

        // 2. BUSCA O PRODUTO NO BANCO
        // Usa a BASE_API definida no topo do script.js ou vazio
        ...
        // 3. PREENCHE DADOS VISUAIS BÁSICOS
        const img = document.getElementById('prod-img');
        if(img) img.src = PRODUTO_ATUAL.imagem || 'https://placehold.co/400';
        
        const title = document.getElementById('prod-title');
        if(title) title.textContent = PRODUTO_ATUAL.titulo;
        
        const cat = document.getElementById('prod-cat');
        if(cat) cat.textContent = `${PRODUTO_ATUAL.categoria || "Peças"} > Ref: ${PRODUTO_ATUAL.referencia || "---"}`;

        // 4. MONTAGEM DA DESCRIÇÃO E APLICAÇÃO INTELIGENTE
        const params = new URLSearchParams(window.location.search);
        const termoPesquisado = (params.get('q') || '').toUpperCase().trim();
        
        let htmlDescricao = "";
        if (PRODUTO_ATUAL.referencia) htmlDescricao += `<strong>Ref:</strong> ${PRODUTO_ATUAL.referencia}<br>`;
        if (PRODUTO_ATUAL.fabricante) htmlDescricao += `<strong>Marca:</strong> ${PRODUTO_ATUAL.fabricante}<br>`;

        // --- INÍCIO DA LÓGICA DE APLICAÇÃO ---
        if (PRODUTO_ATUAL.carros) {
            let listaOriginal = PRODUTO_ATUAL.carros.split(',').map(c => c.trim());
            let aplicacaoFinalHTML = "";
            let destaqueHTML = "";

            // Lógica do UP (Regex Word Boundary)
            const carroParaDestacar = LISTA_CARROS.find(carro => {
                const regex = new RegExp(`\\b${carro}\\b`, 'i');
                return regex.test(termoPesquisado) && PRODUTO_ATUAL.carros.toUpperCase().includes(carro);
            });

            if (carroParaDestacar) {
                const idx = listaOriginal.findIndex(c => c.toUpperCase() === carroParaDestacar);
                if (idx !== -1) listaOriginal.splice(idx, 1);
                
                const anoInfo = PRODUTO_ATUAL.ano ? ` (${PRODUTO_ATUAL.ano})` : "";
                destaqueHTML = `<strong style="color: #e67e22; background: #fff4e6; padding: 2px 6px; border-radius: 4px; border: 1px solid #ffcc80; display: inline-block; margin-bottom: 5px;">${carroParaDestacar}${anoInfo}</strong>`;
            }

            if (listaOriginal.length > 0) {
                const textoRestante = listaOriginal.join(', ');
                if (textoRestante.length > 50) {
                    aplicacaoFinalHTML = `
                        ${destaqueHTML} 
                        <span id="app-restante" style="display: none; color: #666;">${destaqueHTML ? ', ' : ''}${textoRestante}</span>
                        <span id="btn-ver-mais-app" onclick="toggleMaisCarros()" style="color: #3498db; cursor: pointer; font-weight: bold; font-size: 0.85rem; margin-left: 5px;">
                            ... ver mais
                        </span>`;
                } else {
                    aplicacaoFinalHTML = destaqueHTML + (destaqueHTML && textoRestante ? ", " : "") + textoRestante;
                }
            } else {
                aplicacaoFinalHTML = destaqueHTML;
            }

            htmlDescricao += `<div style="margin-top: 10px;"><strong>Aplicação:</strong> ${aplicacaoFinalHTML}</div>`;
        }

        if (PRODUTO_ATUAL.motor) htmlDescricao += `<strong>Motor:</strong> ${PRODUTO_ATUAL.motor}`;

        const descElement = document.getElementById('prod-desc');
        if(descElement) descElement.innerHTML = htmlDescricao || "Sem descrição técnica.";

        // 5. ESTOQUE
        const elEstoque = document.getElementById('prod-estoque');
        const estoqueAtual = parseInt(PRODUTO_ATUAL.estoque || 0);
        if(elEstoque) {
            if (estoqueAtual <= 0) {
                elEstoque.innerHTML = '<span style="color:#c0392b"><i class="ph ph-x-circle"></i> Esgotado</span>';
            } else {
                elEstoque.innerHTML = `<span style="color:#27ae60"><i class="ph ph-check-circle"></i> Disponível (${estoqueAtual} em estoque)</span>`;
            }
        }
        ...
        console.error("Erro carregarProduto:", e);
    }
}
...
function atualizarPrecoTela() {
    ...
    const afData = localStorage.getItem('afiliadoLogado');
    const estaLogado = !!afData; 

    const btnComprar = document.querySelector('.btn-buy-now');
    const boxPreco = document.getElementById('prod-price');
    const boxLucro = document.getElementById('affiliate-area'); 
    const boxParcelas = document.querySelector('.detail-installments');
    const displayLucroDetalhado = document.getElementById('display-lucro-afiliado');

    // SE NÃO TIVER LOGADO
    if (!estaLogado) {
        if(boxPreco) boxPreco.innerHTML = '<span style="font-size:1.1rem; color:#7f8c8d;"><i class="ph ph-lock-key"></i> Login p/ ver preço</span>';
        const oldPrice = document.getElementById('prod-old-price');
        if(oldPrice) oldPrice.textContent = "";
        
        if(boxParcelas) boxParcelas.style.display = 'none';
        if(boxLucro) boxLucro.style.display = 'none';

        if(btnComprar) {
            btnComprar.innerHTML = '<i class="ph ph-sign-in"></i> Entrar para Comprar';
            btnComprar.style.background = '#95a5a6'; 
            btnComprar.onclick = () => window.location.href = 'afiliado_login.html';
        }
        return;
    }
    ...
    // SE TIVER LOGADO
    if(btnComprar) {
        btnComprar.innerHTML = '<i class="ph ph-shopping-cart-plus"></i> Adicionar ao Carrinho';
        btnComprar.style.background = ''; 
        btnComprar.onclick = adicionarAoCarrinhoAtual;
    }
    if(boxParcelas) boxParcelas.style.display = 'block';

    // CÁLCULOS FINANCEIROS
    const precoBase = parseFloat(PRODUTO_ATUAL.preco_novo);
    ...
    // 2. Lucro Bruto
    const lucroBrutoUnitario = precoUnitarioFinal - precoBase;
    
    // 3. Taxas (30% do Lucro Bruto)
    const FATOR_TAXAS = 0.30; 
    const taxasUnitarias = lucroBrutoUnitario * FATOR_TAXAS;
    
    // 4. Lucro Líquido
    const lucroLiquidoUnitario = lucroBrutoUnitario - taxasUnitarias;

    // Totais
    const totalBruto = lucroBrutoUnitario * qtd;
    const totalTaxas = taxasUnitarias * qtd;
    const totalLiquido = lucroLiquidoUnitario * qtd;

    if(boxPreco) boxPreco.textContent = precoUnitarioFinal.toLocaleString('pt-BR', {style: 'currency', currency: 'BRL'});
    
    // Painel Detalhado
    if(displayLucroDetalhado) {
        displayLucroDetalhado.innerHTML = `
            <div style="display:flex; justify-content:space-between; color:#7f8c8d;">
                <span>Ganho Bruto:</span>
                <span>${totalBruto.toLocaleString('pt-BR', {style: 'currency', currency: 'BRL'})}</span>
            </div>
            <div style="display:flex; justify-content:space-between; color:#c0392b; font-size:0.85rem;">
                <span>- Taxas/Imp (Est.):</span>
                <span>${totalTaxas.toLocaleString('pt-BR', {style: 'currency', currency: 'BRL'})}</span>
            </div>
            <div style="margin-top:5px; padding-top:5px; border-top:1px dashed #ddd; display:flex; justify-content:space-between; color:#27ae60; font-weight:bold; font-size:1.1rem;">
                <span>Líquido:</span>
                <span>${totalLiquido.toLocaleString('pt-BR', {style: 'currency', currency: 'BRL'})}</span>
            </div>
            <div style="text-align:right; font-size:0.7rem; color:#aaa; margin-top:2px;">
                (${qtd} itens)
            </div>
        `;
    }
    ...
    const oldPrice = document.getElementById('prod-old-price');
    if(oldPrice) {
        if(PRODUTO_ATUAL.preco_antigo && PRODUTO_ATUAL.preco_antigo > precoBase) {
            oldPrice.textContent = parseFloat(PRODUTO_ATUAL.preco_antigo).toLocaleString('pt-BR', {style: 'currency', currency: 'BRL'});
        } else {
...
function adicionarAoCarrinhoAtual() {
    ...
    const estoqueMax = parseInt(PRODUTO_ATUAL.estoque || 0);
    const inputQtd = document.getElementById('qtd');
    const qtdQueroAdicionar = parseInt(inputQtd.value);
    ...
    if (index > -1) {
        qtdJaNoCarrinho = carrinho[index].quantidade;
    }

    if ((qtdJaNoCarrinho + qtdQueroAdicionar) > estoqueMax) {
        alert(`⚠️ ESTOQUE INSUFICIENTE!\n\nVocê já tem ${qtdJaNoCarrinho} no carrinho e quer adicionar +${qtdQueroAdicionar}.\nSó temos ${estoqueMax} unidades no total.`);
        return; 
    }

    const urlParams = new URLSearchParams(window.location.search);
    const termoBusca = urlParams.get('q') || urlParams.get('search') || ''; 

    const dadosExtras = {
        tituloOriginal: PRODUTO_ATUAL.titulo,
        referencia: PRODUTO_ATUAL.referencia || '',
        listaCarros: PRODUTO_ATUAL.carros || '',
        termoPesquisa: termoBusca
    };

    if (index > -1) {
        carrinho[index].quantidade += qtdQueroAdicionar;
        carrinho[index].customMargin = MARGEM_ATUAL;
        carrinho[index].termoPesquisa = termoBusca; 
    } else {
        carrinho.push({ 
            id: PRODUTO_ATUAL.id, 
            quantidade: qtdQueroAdicionar, 
            customMargin: MARGEM_ATUAL,
            preco: PRODUTO_ATUAL.preco_novo, 
    ...
    // Se existir a função de atualizar ícone no header, chama ela
...
async function enviarSugestao() {
    const termoInput = document.getElementById('termo-sugestao');
    const termo = termoInput.value;
    if(!termo) return alert("Digite algo!");
    
    const afLogadoStr = localStorage.getItem('afiliadoLogado');
    if(!afLogadoStr) return alert("Você precisa estar logado novamente para sugerir.");
    
    const afLogado = JSON.parse(afLogadoStr);
    ...
    try {
        const apiUrl = (typeof API_URL !== 'undefined') ? API_URL : ''; 
        const res = await fetch(`${apiUrl}/afiliado/sugestoes`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${afLogado.token}`
            },
            body: JSON.stringify({
                produtoId: PRODUTO_ATUAL.id,
                termo: termo,
                motivo: "Sugestão via página do produto"
            })
        });

        if(res.ok) {
            alert("✅ Obrigado! Sua sugestão foi enviada para o Admin.");
...
function toggleMaisCarros() {
    const spanRestante = document.getElementById('app-restante');
    const btn = document.getElementById('btn-ver-mais-app');
